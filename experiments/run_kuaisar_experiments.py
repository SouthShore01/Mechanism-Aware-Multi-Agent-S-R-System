...
Runs all four experiments on real Kuaishou search-and-recommendation data:
...
def main():
    ...
    parser.add_argument("--exp",        type=str,   default="all",
                        help="Which experiments to run: all | 1 | 2 | 3 | 4 | 1,2,3")
    ...
    if 1 in exps:
        metrics, last_fixed_outputs = run_exp1(
    ...
    if 3 in exps:
        if last_fixed_outputs is None:
            # Build outputs from first query for monotonicity test
            q_info = queries[0]
            user   = users[0]
            uctx   = {"user_id": user["user_id"], "history": user["history"],
                      "preferred_categories": user["preferred_categories"]}
